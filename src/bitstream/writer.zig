...
pub fn Writer(comptime WriterType: type) type {
    return struct {
        ...
        pub fn writeAbbrevId(self: *Self, id: codes.abbrev.Id, width: u5) !void {
            try self.writeInt(u32, @as(u32, @enumToInt(id)), width);
        }

        pub fn writeAbbrevOp(self: *Self, op: bitstream.AbbrevOp) !void {
            switch (op) {
                .constant => |c| {
                    try self.writeIntAuto(@as(u1, 1));
                    try self.writeVbr(c, 8);
                    return;
                },
                else => {
                    try self.writeIntAuto(@as(u1, 0));
                },
            }
            switch (op) {
                .constant => unreachable,
                .fixed => |width| {
                    try self.writeIntAuto(@enumToInt(codes.abbrev.OpEncoding.fixed));
                    try self.writeVbr(width, 5);
                },
                .vbr => |width| {
                    try self.writeIntAuto(@enumToInt(codes.abbrev.OpEncoding.vbr));
                    try self.writeVbr(width, 5);
                },
                .char6 => {
                    try self.writeIntAuto(@enumToInt(codes.abbrev.OpEncoding.char6));
                },
                .blob => {
                    try self.writeIntAuto(@enumToInt(codes.abbrev.OpEncoding.blob));
                },
                .array => {
                    try self.writeIntAuto(@enumToInt(codes.abbrev.OpEncoding.array));
                },
            }
        }
    };
